	...
	for prop in ProjectSettings.get_property_list():
		if prop.name.begins_with("autoload/"):
			...
			suggestions[autoload] = {
				"value": autoload, "tooltip": autoload, "editor_icon": ["Node", "EditorIcons"]
			}
...
func _get_code_completion(
	_CodeCompletionHelper: Node, TextNode: TextEdit, line: String, _word: String, symbol: String
) -> void:
	if line.count(" ") == 1 and not "." in line:
		...
		for i in get_method_suggestions("", line.get_slice(".", 0).trim_prefix("do ")):
			TextNode.add_code_completion_option(
				...
				i + "(",
				event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.3),
				TextNode.get_theme_icon("Callable", "EditorIcons")
			)
