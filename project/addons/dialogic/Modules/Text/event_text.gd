...
func _execute() -> void:
	...
	if (
		(not character or character.custom_info.get("style", "").is_empty())
		and dialogic.has_subsystem("Styles")
	):
		# if previous characters had a custom style change back to base style
		if (
			dialogic.current_state_info.get("base_style")
			!= dialogic.current_state_info.get("style")
		):
	...
	if character:
		...
		var check_portrait: String = (
			portrait
			if !portrait.is_empty()
			else dialogic.current_state_info["portraits"].get(character.resource_path, {}).get(
				"portrait", ""
			)
		)
		...
		if (
			check_portrait
			and (
				character.portraits.get(check_portrait, {}).get("sound_mood", "")
				in character.custom_info.get("sound_moods", {})
			)
		):
			dialogic.Text.update_typing_sound_mood(
				character.custom_info.get("sound_moods", {}).get(
					character.portraits[check_portrait].get("sound_mood", {}), {}
				)
			)
		elif !character.custom_info.get("sound_mood_default", "").is_empty():
			dialogic.Text.update_typing_sound_mood(
				character.custom_info.get("sound_moods", {}).get(
					character.custom_info.get("sound_mood_default"), {}
				)
			)
		else:
			dialogic.Text.update_typing_sound_mood()
	...
	if result and not result.get_string("name").is_empty():
		...
		if name == "_":
			...
			if character == null and Engine.is_editor_hint() == false:
				...
				DialogicResourceUtil.add_resource_to_directory(
					character.resource_path, DialogicResourceUtil.get_character_directory()
				)
