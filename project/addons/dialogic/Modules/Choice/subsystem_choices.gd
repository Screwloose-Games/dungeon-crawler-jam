...
func get_current_question_info() -> Dictionary:
	...
	for choice_index in get_current_choice_indexes():
		...
		choice_info.text = dialogic.Text.parse_text(
			choice_info.text, true, true, false, true, false, false
		)
...
func _on_dialogic_action() -> void:
	if (
		get_viewport().gui_get_focus_owner() is DialogicNode_ChoiceButton
		and use_input_action
		and not dialogic.Inputs.input_was_mouse_input
	):
		...
		if index != 0 and dialogic.current_timeline_events[index - 1] is DialogicEndBranchEvent:
			if (
				dialogic.current_timeline_events[
					dialogic.current_timeline_events[index - 1].find_opening_index(index - 1)
				]
				is DialogicChoiceEvent
			):
				return false
			else:
				return true
