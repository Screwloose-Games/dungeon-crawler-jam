	...
	if not info.is_empty() and info.has("path"):
		...
		for channel_id in info.keys():
			if info[channel_id].is_empty() or info[channel_id].path.is_empty():
				...
				update_music(
					info[channel_id].path,
					info[channel_id].volume,
					info[channel_id].audio_bus,
					0,
					info[channel_id].loop,
					channel_id
				)
...
func update_music(
	path := "", volume := 0.0, audio_bus := "", fade_time := 0.0, loop := true, channel_id := 0
) -> void:
	if channel_id > max_channels:
		printerr(
			"\tChannel ID (%s) higher than Max Music Channels (%s)" % [channel_id, max_channels]
		)
	...
	dialogic.current_state_info["music"][channel_id] = {
		"path": path, "volume": volume, "audio_bus": audio_bus, "loop": loop, "channel": channel_id
	}
	...
	if (
		(
			is_instance_valid(current_music_player[channel_id])
			and current_music_player[channel_id].playing
		)
		or !path.is_empty()
	):
		fader = create_tween()
	...
	if (
		...
		fader.tween_method(
			interpolate_volume_linearly.bind(prev_node),
			db_to_linear(prev_node.volume_db),
			0.0,
			fade_time
		)
	...
	if path:
		...
		fader.parallel().tween_method(
			interpolate_volume_linearly.bind(current_music_player[channel_id]),
			0.0,
			db_to_linear(volume),
			fade_time
		)
...
func has_music(channel_id := 0) -> bool:
	return !(
		dialogic.current_state_info.get("music", {}).get(channel_id, {}).get("path", "").is_empty()
	)
