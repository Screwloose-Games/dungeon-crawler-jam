		...
		if symbol == "[":
			...
			# suggest values
				...
				if !shortcode_events[code].get_shortcode_parameters().has(current_parameter):
					text.update_code_completion_options(false)
					return
				if !shortcode_events[code].get_shortcode_parameters()[current_parameter].has(
					"suggestions"
				):
					if (
						typeof(
							(
								shortcode_events[code]
								. get_shortcode_parameters()[current_parameter]
								. default
							)
						)
						== TYPE_BOOL
				...
				var suggestions: Dictionary = (
					shortcode_events[code]
					. get_shortcode_parameters()[current_parameter]["suggestions"]
					. call()
				)
...
func suggest_characters(
	text: CodeEdit, type := CodeEdit.KIND_MEMBER, text_event_start := false
) -> void:
