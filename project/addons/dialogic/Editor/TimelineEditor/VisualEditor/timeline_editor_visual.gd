...
func _search_timeline(search_text: String) -> bool:
	...
	for block in %Timeline.get_children():
		if block.resource is DialogicTextEvent:
			var text_field: TextEdit = block.get_node("%BodyContent").find_child(
				"Field_Text_Multiline", true, false
			)
...
func search_navigate(navigate_up := false) -> void:
	...
	while not selected_items[0] in search_results:
		select_item(
			%Timeline.get_child(
				wrapi(selected_items[0].get_index() + 1, 0, %Timeline.get_child_count() - 1)
			),
			false
		)
...
func search_text_field(field: TextEdit, search_text := "", navigate_up := false) -> Vector2i:
	...
	var search := field.search(
		search_text, 4 if navigate_up else 0, search_from_line, search_from_column
	)
	return search
