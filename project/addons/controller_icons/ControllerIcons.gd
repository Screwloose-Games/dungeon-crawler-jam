...
enum InputType { KEYBOARD_MOUSE, CONTROLLER }  ## The input is from the keyboard and/or mouse.  ## The input is from a controller.

enum PathType { INPUT_ACTION, JOYPAD_PATH, SPECIFIC_PATH }  ## The path is an input action.  ## The path is a generic joypad path.  ## The path is a specific path.
...
func parse_path(
	path: String, input_type = _last_input_type, last_controller = _last_controller
) -> Texture:
...
func parse_path_to_tts(
	path: String, input_type: int = _last_input_type, controller: int = _last_controller
) -> String:
	if input_type == null:
		return ""
...
func get_matching_event(
	path: String, input_type: InputType = _last_input_type, controller: int = _last_controller
) -> InputEvent:
	var events: Array
