...
func build_tree(editor_interface: EditorInterface):
	selector = (
		preload("res://addons/controller_icons/objects/ControllerIconPathSelectorPopup.tscn")
		. instantiate()
	)
	...
	line_edit.text_changed.connect(func(text): emit_changed(get_edited_property(), text))
