...
var path_selector := preload(
	"res://addons/controller_icons/objects/ControllerIconPathEditorProperty.gd"
)
...
class ControllerIcons_TexturePreview:
	...
	func _init(editor_interface: EditorInterface):
		...
		background = editor_interface.get_base_control().get_theme_icon(
			"Checkerboard", "EditorIcons"
		)
