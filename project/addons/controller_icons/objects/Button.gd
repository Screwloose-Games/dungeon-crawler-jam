...
func _on_input_type_changed(input_type, controller):
	if (
		show_only == 0
		or (show_only == 1 and input_type == ControllerIcons.InputType.KEYBOARD_MOUSE)
		or (show_only == 2 and input_type == ControllerIcons.InputType.CONTROLLER)
	):
		self.path = path
	else:
		icon = null
...
func get_tts_string() -> String:
	if force_type:
		return ControllerIcons.parse_path_to_tts(
			path, force_type - 1, ControllerIcons._last_controller
		)
	else:
