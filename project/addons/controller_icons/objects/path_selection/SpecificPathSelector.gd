...
func populate(editor_interface: EditorInterface) -> void:
	...
	var base_paths := [
		ControllerIcons._settings.custom_asset_dir, "res://addons/controller_icons/assets"
	]
...
func create_icon(category: String, path: String):
	...
	if button_nodes[map_category].has(filename):
		return
...
func _on_base_asset_names_item_selected():
	...
	if not button_nodes.has(category):
		return
...
func _on_name_filter_text_changed(new_text: String):
	...
	while asset_name:
		...
		if any_visible.has(category):
			...
			asset_name.set_custom_color(
				0, color_text_enabled if selectable else color_text_disabled
			)
		asset_name = asset_name.get_next_in_tree()
