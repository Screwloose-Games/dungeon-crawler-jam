...
func populate(editor_interface: EditorInterface) -> void:
	...
	n_name_filter.right_icon = editor_interface.get_base_control().get_theme_icon(
		"Search", "EditorIcons"
	)
	...
	var default_actions := ControllerIcons._builtin_keys.map(
		func(value: String): return value.trim_prefix("input/")
	)
...
func _on_name_filter_text_changed(new_text: String):
	...
	for item in items:
		var filtered := (
			true if new_text.is_empty() else item.tree_item.get_text(0).findn(new_text) != -1
		)
