...
func _convert_joypad_path(
	path: String, device: int, fallback: ControllerSettings.Devices
) -> String:
...
func _get_joypad_type(device, fallback):
	...
	if "Luna Controller" in controller_name:
	...
	elif "PS4 Controller" in controller_name or "DUALSHOCK 4" in controller_name:
		return ControllerSettings.Devices.PS4
	elif "PS5 Controller" in controller_name or "DualSense" in controller_name:
	...
	elif "Switch Controller" in controller_name or "Switch Pro Controller" in controller_name:
		...
		"Xbox One" in controller_name
		or "X-Box One" in controller_name
		or "Xbox Wireless Controller" in controller_name
	):
		return ControllerSettings.Devices.XBOXONE
	elif "Xbox Series" in controller_name:
		return ControllerSettings.Devices.XBOXSERIES
	elif "Steam Deck" in controller_name or "Steam Virtual Gamepad" in controller_name:
