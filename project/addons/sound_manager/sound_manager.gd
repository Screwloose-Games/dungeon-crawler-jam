...
var ui_sound_effects: SoundEffectsPlayer = SoundEffectsPlayer.new(
	["UI", "Interface", "Sounds", "SFX"], 8
)
...
func get_ui_sound_volume() -> float:
	return db_to_linear(
		AudioServer.get_bus_volume_db(AudioServer.get_bus_index(ui_sound_effects.bus))
	)
...
func set_sound_volume(volume_between_0_and_1: float) -> void:
	...
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(sound_effects.bus), linear_to_db(volume_between_0_and_1)
	)
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(ui_sound_effects.bus), linear_to_db(volume_between_0_and_1)
	)
...
func play_sound_with_pitch(
	resource: AudioStream, pitch: float = 1.0, override_bus: String = ""
) -> AudioStreamPlayer:
...
func get_ambient_sound_volume() -> float:
	return db_to_linear(
		AudioServer.get_bus_volume_db(AudioServer.get_bus_index(ambient_sounds.bus))
	)
...
func set_ambient_sound_volume(volume_between_0_and_1: float) -> void:
	_show_shared_bus_warning()
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(ambient_sounds.bus), linear_to_db(volume_between_0_and_1)
	)


func play_ambient_sound(
	resource: AudioStream, fade_in_duration: float = 0.0, override_bus: String = ""
) -> AudioStreamPlayer:
...
func set_music_volume(volume_between_0_and_1: float) -> void:
	_show_shared_bus_warning()
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(music.bus), linear_to_db(volume_between_0_and_1)
	)


func play_music(
	resource: AudioStream, crossfade_duration: float = 0.0, override_bus: String = ""
) -> AudioStreamPlayer:
	return music.play(resource, 0.0, 0.0, crossfade_duration, override_bus)


func play_music_from_position(
...
func play_music_from_position_at_volume(
	resource: AudioStream,
	position: float = 0.0,
	volume: float = 0.0,
	crossfade_duration: float = 0.0,
