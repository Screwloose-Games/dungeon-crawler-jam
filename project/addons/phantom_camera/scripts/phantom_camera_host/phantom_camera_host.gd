...
func _ready() -> void:
	...
	if Engine.has_singleton(_constants.PCAM_MANAGER_NODE_NAME):
		...
		printerr(
			"Make sure the addon is enable or that the singleton hasn't been disabled inside Project Settings / Globals"
		)
...
func _assign_new_active_pcam(pcam: Node) -> void:
	...
	if is_instance_valid(_active_pcam_2d) or is_instance_valid(_active_pcam_3d):
		...
		if _is_2d:
			...
			if _active_pcam_3d.camera_3d_resource_property_changed.is_connected(
				_camera_3d_resource_property_changed
			):
				_active_pcam_3d.camera_3d_resource_property_changed.disconnect(
					_camera_3d_resource_property_changed
				)
				...
				if camera_3d.attributes is CameraAttributesPractical:
					...
					if _attributes.auto_exposure_enabled:
						_prev_cam_exposure_max_sensitivity = (
							_attributes.auto_exposure_max_sensitivity
						)
						_prev_cam_exposure_min_sensitivity = (
							_attributes.auto_exposure_min_sensitivity
						)
					...
					if _attributes.auto_exposure_enabled:
						_prev_cam_exposure_min_exposure_value = (
							_attributes.auto_exposure_min_exposure_value
						)
						_prev_cam_exposure_max_exposure_value = (
							_attributes.auto_exposure_max_exposure_value
						)
	...
	if _is_2d:
		...
		if not _active_pcam_3d.camera_3d_resource_property_changed.is_connected(
			_camera_3d_resource_property_changed
		):
			_active_pcam_3d.camera_3d_resource_property_changed.connect(
				_camera_3d_resource_property_changed
			)
		...
		if _active_pcam_3d.camera_3d_resource:
			...
			if Engine.is_editor_hint():
				if not (
					...
					. get_inspector()
					. property_edited
					. is_connected(_camera_3d_edited)
				):
					(
						Engine
						. get_singleton(&"EditorInterface")
	...
	if _is_2d:
		...
		if Engine.get_version_info().major == 4 and Engine.get_version_info().minor >= 4:
			if (
				...
				or _active_pcam_3d.get_follow_target_physics_based()
			):
...
func _tween_follow_checker(delta: float) -> void:
	...
	if _is_2d:
		if not _has_noise_emitted and not _active_pcam_2d.has_noise_resource():
			return
		camera_2d.offset += (
			_active_pcam_2d.get_noise_transform().origin + _noise_emitted_output_2d.origin
		)
		if camera_2d.ignore_rotation and _noise_emitted_output_2d.get_rotation() != 0:
			push_warning(
				camera_2d.name,
				" has ignore_rotation enabled. Uncheck the property if you want to apply rotational noise."
			)
		else:
			camera_2d.rotation += (
				_active_pcam_2d.get_noise_transform().get_rotation()
				+ _noise_emitted_output_2d.get_rotation()
			)
		...
		if not _has_noise_emitted and not _active_pcam_3d.has_noise_resource():
			return
		camera_3d.global_transform *= (
			_active_pcam_3d.get_noise_transform() * _noise_emitted_output_3d
		)
		_has_noise_emitted = false
	...
	if _active_pcam_3d.camera_3d_resource:
		if Engine.is_editor_hint():
			if not (
				...
				Engine.get_singleton(&"EditorInterface").get_inspector().property_edited.connect(
					_camera_3d_edited
				)
		camera_3d.keep_aspect = _active_pcam_3d.keep_aspect
		camera_3d.cull_mask = _active_pcam_3d.cull_mask
		...
		if Engine.is_editor_hint():
			if (
				...
				Engine.get_singleton(&"EditorInterface").get_inspector().property_edited.disconnect(
					_camera_3d_edited
				)
...
func _camera_3d_edited(value: String) -> void:
	if not (
		Engine.get_singleton(&"EditorInterface").get_inspector().get_edited_object() == camera_3d
	):
		return
	camera_3d.set(value, _active_pcam_3d.camera_3d_resource.get(value))
	push_warning(
		"Camera3D properties are being overridden by ", _active_pcam_3d.name, "'s Camera3DResource"
	)
...
func _pcam_tween(delta: float) -> void:
	...
	if _is_2d:
		...
		var prev_active_pcam_3d_quat: Quaternion = Quaternion(
			_prev_active_pcam_3d_transform.basis.orthonormalized()
		)
		camera_3d.quaternion = Tween.interpolate_value(
			prev_active_pcam_3d_quat,
			(
				prev_active_pcam_3d_quat.inverse()
				* Quaternion(_active_pcam_3d_glob_transform.basis.orthonormalized())
			),
			_tween_elapsed_time,
			_active_pcam_3d.tween_duration,
			_active_pcam_3d.tween_transition,
		...
		if _cam_h_offset_changed:
			camera_3d.h_offset = _tween_interpolate_value(
				_prev_cam_h_offset,
		...
		if _cam_v_offset_changed:
			camera_3d.v_offset = _tween_interpolate_value(
				_prev_cam_v_offset,
		...
		if _cam_fov_changed:
			camera_3d.fov = _tween_interpolate_value(
				_prev_cam_fov,
				_active_pcam_3d.fov,
				_active_pcam_3d.tween_duration,
				_active_pcam_3d.tween_transition,
		...
		if _cam_size_changed:
			camera_3d.size = _tween_interpolate_value(
				_prev_cam_size,
		...
		if _cam_frustum_offset_changed:
			camera_3d.frustum_offset = _tween_interpolate_value(
		...
		if _cam_near_changed:
			camera_3d.near = _tween_interpolate_value(
				_prev_cam_near,
		...
		if _cam_far_changed:
			camera_3d.far = _tween_interpolate_value(
				_prev_cam_far,
				_active_pcam_3d.far,
				_active_pcam_3d.tween_duration,
				_active_pcam_3d.tween_transition,
...
func _tween_interpolate_value(
	from: Variant, to: Variant, duration: float, transition_type: int, ease_type: int
) -> Variant:
	return (
		Tween
		. interpolate_value(
...
func _show_viewfinder_in_play() -> void:
	...
	if not is_instance_valid(_viewfinder_node):
		var _viewfinder_scene := load(
			"res://addons/phantom_camera/panel/viewfinder/viewfinder_panel.tscn"
		)
