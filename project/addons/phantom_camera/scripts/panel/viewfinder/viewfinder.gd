...
const _camera_2d_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/viewfinder/Camera2DIcon.svg"
)
const _camera_3d_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/viewfinder/Camera3DIcon.svg"
)
const _pcam_host_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/phantom_camera_host.svg"
)
const _pcam_2D_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/phantom_camera_2d.svg"
)
const _pcam_3D_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/phantom_camera_3d.svg"
)
...
var _no_open_scene_icon: CompressedTexture2D = preload(
	"res://addons/phantom_camera/icons/viewfinder/SceneTypesIcon.svg"
)
...
func _process(_delta: float) -> void:
	...
	if not Engine.is_editor_hint():
		target_point.position = (
			camera_viewport_panel.size * unprojected_position_clamped - target_point.size / 2
		)
	...
	var window_size_height: float = ProjectSettings.get_setting(
...
func _settings_changed() -> void:
	...
	sub_viewport.canvas_item_default_texture_filter = ProjectSettings.get_setting(
...
func _visibility_check() -> void:
	...
	if not (
		Engine
		. get_singleton(_constants.PCAM_MANAGER_NODE_NAME)
		. get_phantom_camera_hosts()
		. is_empty()
	):
		has_camera = true
		pcam_host = (
			Engine.get_singleton(_constants.PCAM_MANAGER_NODE_NAME).get_phantom_camera_hosts()[0]
		)
				...
				if pcam_host:
					if (
						(
							get_tree()
							. root
							. get_node(_constants.PCAM_MANAGER_NODE_NAME)
							. get_phantom_camera_2ds()
						)
						or (
							get_tree()
							. root
							. get_node(_constants.PCAM_MANAGER_NODE_NAME)
							. get_phantom_camera_3ds()
						)
					):
						# Pcam exists in tree
...
#					No PCamHost in scene
					_update_button(
						_constants.PCAM_HOST_NODE_NAME, _pcam_host_icon, _constants.PCAM_HOST_COLOR
					)
...
func _update_button(text: String, icon: CompressedTexture2D, color: Color) -> void:
	_add_node_button_text.set_text(
		"[center]Add [img=32]" + icon.resource_path + "[/img] [b]" + text + "[/b][/center]"
	)
...
func _add_node(node_type: String) -> void:
	...
	match node_type:
		...
		_constants.PCAM_HOST_NODE_NAME:
			...
			_instantiate_node(scene_root, pcam_host, _constants.PCAM_HOST_NODE_NAME, camera_owner)
...
func _instantiate_node(
	scene_root: Node, node: Node, name: String, parent: Node = scene_root
) -> void:
	node.set_name(name)
...
func _set_viewfinder(root: Node, editor: bool) -> void:
	pcam_host_group = (
		get_tree().root.get_node(_constants.PCAM_MANAGER_NODE_NAME).get_phantom_camera_hosts()
	)
...
func _set_viewfinder_camera(new_pcam_host: PhantomCameraHost, editor: bool) -> void:
	...
	if _is_2d:
		...
		if _selected_camera.keep_aspect == Camera3D.KeepAspect.KEEP_HEIGHT:
			aspect_ratio_container.set_stretch_mode(
