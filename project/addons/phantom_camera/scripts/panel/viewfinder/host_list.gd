...
const _host_list_item: PackedScene = preload(
	"res://addons/phantom_camera/panel/viewfinder/host_list/host_list_item.tscn"
)
...
func _host_list_button_pressed() -> void:
	...
	var duration: float = clampf(
		max_duration / (300 / _host_list_item_container.size.y), 0.3, max_duration
	)

	(
		tween
		. tween_property(self, "offset_top", _set_host_list_size(), duration)
