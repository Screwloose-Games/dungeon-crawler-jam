...
func _follow(delta: float) -> void:
	match follow_mode:
			...
			if _has_multiple_follow_targets:
				...
				if auto_zoom:
					...
					if rect.size.x > rect.size.y * _phantom_camera_manager.screen_size.aspect():
						zoom = (
							clamp(
								_phantom_camera_manager.screen_size.x / rect.size.x,
								auto_zoom_min,
								auto_zoom_max
							)
							* Vector2.ONE
						...
						zoom = (
							clamp(
								_phantom_camera_manager.screen_size.y / rect.size.y,
								auto_zoom_min,
								auto_zoom_max
							)
							* Vector2.ONE
		...
		FollowMode.PATH:
			...
			_target_transform.origin = (
				follow_path.curve.get_closest_point(_target_position_with_offset() - path_position)
				+ path_position
			)
		...
		FollowMode.FRAMED:
			if not Engine.is_editor_hint():
				viewport_position = (
					(
						get_follow_target().get_global_transform_with_canvas().get_origin()
						+ follow_offset
					)
				...
				if framed_side_offset != Vector2.ZERO:
					...
					var target_position: Vector2 = (
						_target_position_with_offset() + _follow_framed_offset
					)
					...
					if framed_side_offset.x != 0 and framed_side_offset.y == 0:
						...
						_follow_framed_offset.y = (
							global_position.y - _target_position_with_offset().y
						)
					...
					# If a vertical dead zone is reached
						...
						_follow_framed_offset.x = (
							global_position.x - _target_position_with_offset().x
						)
					...
					_follow_framed_offset = (
						_transform_output.origin - _target_position_with_offset()
					)
					return
			else:
				_target_transform.origin = _target_position_with_offset()
...
func _set_limit_clamp_position(value: Vector2) -> Vector2:
	...
	value.x = clampf(
		value.x,
		_limit_sides.x + camera_frame_rect_size.x / 2,
		_limit_sides.z - camera_frame_rect_size.x / 2
	)
	value.y = clampf(
		value.y,
		_limit_sides.y + camera_frame_rect_size.y / 2,
		_limit_sides.w - camera_frame_rect_size.y / 2
	)
	return value
...
func _camera_frame_rect() -> Rect2:
	var screen_size_zoom: Vector2 = Vector2(
		_phantom_camera_manager.screen_size.x / get_zoom().x,
		_phantom_camera_manager.screen_size.y / get_zoom().y
	)
	...
	if target is PhysicsBody2D:
		...
		if Engine.get_version_info().major == 4 and Engine.get_version_info().minor < 3:
			if show_jitter_tips == null:  # Default value is null when referencing custom Project Setting
				print_rich(
					"Following a [b]PhysicsBody2D[/b] node will likely result in jitter - on lower physics ticks in particular."
				)
				print_rich(
					"If possible, will recommend upgrading to Godot 4.3, as it has built-in support for 2D Physics Interpolation, which will mitigate this issue."
				)
				print_rich(
					"Otherwise, try following the guide on the [url=https://phantom-camera.dev/support/faq#i-m-seeing-jitter-what-can-i-do]documentation site[/url] for better results."
				)
				print_rich(
...
func update_limit_all_sides() -> void:
	...
	if not is_instance_valid(_limit_node):
		...
		var tile_map_size: Vector2 = (
			Vector2(tile_map.get_used_rect().size)
		...
		var tile_map_position: Vector2 = (
			...
			+ (
				Vector2(tile_map.get_used_rect().position)
				* Vector2(tile_map.tile_set.tile_size)
				* tile_map.get_scale()
			)
		)
		...
		limit_rect = Rect2(
			...
			(
				tile_map_size
				- Vector2(limit_margin.x, limit_margin.y)
				- Vector2(limit_margin.z, limit_margin.w)
			)
		)
		...
		var shape_2d_position: Vector2 = (
			collision_shape_2d.global_position + Vector2(shape_2d.get_rect().position)
		)
...
func _limit_target_exist_error() -> void:
	if not limit_target.is_empty():
		printerr(
			"Unable to set Limit Side due to Limit Target ", _limit_node.name, " being assigned"
		)
