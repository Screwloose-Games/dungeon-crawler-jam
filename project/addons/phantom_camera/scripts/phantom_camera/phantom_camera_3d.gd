...
const _constants = preload(
	"res://addons/phantom_camera/scripts/phantom_camera/phantom_camera_constants.gd"
)
		...
		if Engine.is_editor_hint():
			if value:
				if not Engine.has_singleton(_constants.PCAM_MANAGER_NODE_NAME):
					return
				Engine.get_singleton(_constants.PCAM_MANAGER_NODE_NAME).pcam_priority_override.emit(
...
@export var follow_mode: FollowMode = FollowMode.NONE:
	set(value):
		...
		if look_at_mode != LookAtMode.NONE:
			print_rich(
				"[color=#EAA15E]Warning: Using both Look At and Follow Mode on the same PCam3D has not been fully tested yet, proceed with caution![/color]"
			)
	get:
		return follow_mode
...
func _validate_property(property: Dictionary) -> void:
	...
	if property.name == "follow_path" and follow_mode != FollowMode.PATH:
		property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if follow_mode == FollowMode.NONE:
		match property.name:
			"follow_offset", "follow_damping", "follow_damping_value", "follow_axis_lock":
				property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if property.name == "follow_distance":
		if not follow_mode == FollowMode.FRAMED:
			if not follow_mode == FollowMode.GROUP or auto_follow_distance:
				property.usage = PROPERTY_USAGE_NO_EDITOR

	###############
	## Group Follow
	###############
	if property.name == "follow_targets" and not follow_mode == FollowMode.GROUP:
		property.usage = PROPERTY_USAGE_NO_EDITOR

	if property.name == "auto_follow_distance" and not follow_mode == FollowMode.GROUP:
		property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if not follow_mode == FollowMode.THIRD_PERSON:
		match property.name:
			"spring_length", "collision_mask", "shape", "margin":
				property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if look_at_mode == LookAtMode.NONE:
		match property.name:
			"look_at_target", "look_at_offset", "look_at_damping", "look_at_damping_value":
				property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if property.name == "look_at_target":
		if look_at_mode == LookAtMode.NONE or look_at_mode == LookAtMode.GROUP:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	if property.name == "look_at_targets" and not look_at_mode == LookAtMode.GROUP:
		property.usage = PROPERTY_USAGE_NO_EDITOR

	if property.name == "look_at_damping_value" and not look_at_damping:
		property.usage = PROPERTY_USAGE_NO_EDITOR
...
func _follow(delta: float) -> void:
	...
	match follow_mode:
			...
			if _has_multiple_follow_targets:
				...
				if auto_follow_distance:
					distance = lerpf(
						...
						bounds.get_longest_axis_size() / auto_follow_distance_divisor
					)
				...
				_target_transform.origin = (
					bounds.get_center()
					+ follow_offset
					+ global_transform.basis.z * Vector3(distance, distance, distance)
				)
			else:
				_target_transform.origin = (
		...
		FollowMode.PATH:
			...
			_target_transform.origin = (
				follow_path.curve.get_closest_point(follow_target.global_position - path_position)
				+ path_position
			)
		...
		FollowMode.FRAMED:
			if not Engine.is_editor_hint():
				...
				viewport_position = get_viewport().get_camera_3d().unproject_position(
					_get_target_position_offset()
				)
				...
				if _get_framed_side_offset() != Vector2.ZERO:
					var target_position: Vector3 = (
						_get_target_position_offset() + _follow_framed_offset
					)
					var glo_pos: Vector3
					...
					if dead_zone_width == 0 || dead_zone_height == 0:
						...
						if _current_rotation != global_rotation:
							var opposite: float = (
								...
								+ _get_target_position_offset().y
							)
							glo_pos.y = _get_target_position_offset().y + opposite
							glo_pos.z = (
								sqrt(pow(follow_distance, 2) - pow(opposite, 2))
								+ _get_target_position_offset().z
							)
...
func _look_at(delta: float) -> void:
	match look_at_mode:
		LookAtMode.MIMIC:
			_interpolate_rotation(global_position - look_at_target.global_basis.z, delta)

		LookAtMode.SIMPLE:
			_interpolate_rotation(look_at_target.global_position, delta)

		LookAtMode.GROUP:
			if not _has_multiple_look_at_targets:
				_interpolate_rotation(
					look_at_targets[_look_at_targets_single_target_index].global_position, delta
				)
			else:
				...
				_interpolate_rotation(bounds.get_center(), delta)
...
func _interpolate_position(
	target_position: Vector3, delta: float, camera_target: Node3D = self
) -> void:
	if follow_damping:
...
func _get_raw_unprojected_position() -> Vector2:
	return get_viewport().get_camera_3d().unproject_position(
		follow_target.global_position + follow_offset
	)
	...
	for i in follow_targets.size():
		...
		if follow_targets[i].is_inside_tree():
			...
			if not follow_targets[i].tree_exiting.is_connected(_follow_target_tree_exiting):
				follow_targets[i].tree_exiting.connect(
					_follow_target_tree_exiting.bind(follow_targets[i])
				)
	...
	for i in look_at_targets.size():
		if is_instance_valid(look_at_targets[i]):
			...
			if not look_at_targets[i].tree_exiting.is_connected(_look_at_target_tree_exiting):
				look_at_targets[i].tree_exiting.connect(
					_look_at_target_tree_exiting.bind(look_at_targets[i])
				)
...
func _check_physics_body(target: Node3D) -> void:
	if target is PhysicsBody3D:
		...
		var physics_interpolation_enabled := ProjectSettings.get_setting(
		...
		if Engine.get_version_info().major == 4 and Engine.get_version_info().minor < 4:
			if show_jitter_tips == null:  # Default value is null when referencing custom Project Setting
				print_rich(
					"Following or Looking at a [b]PhysicsBody3D[/b] node will likely result in jitter - on lower physics ticks in particular."
				)
				print_rich(
					"If possible, will recommend upgrading to Godot 4.4, as it has built-in support for 3D Physics Interpolation, which will mitigate this issue."
				)
				print_rich(
					"Until then, try following the guide on the [url=https://phantom-camera.dev/support/faq#i-m-seeing-jitter-what-can-i-do]documentation site[/url] for better results."
				)
				print_rich(
					"This tip can be disabled from within [code]Project Settings / Phantom Camera / Tips / Show Jitter Tips[/code]"
				)
			return
		## NOTE - Only supported in Godot 4.4 or above
			...
			printerr(
				"Physics Interpolation is disabled in the Project Settings, recommend enabling it to smooth out physics-based camera movement"
			)
			print_rich(
				"This tip can be disabled from within [code]Project Settings / Phantom Camera / Tips / Show Jitter Tips[/code]"
			)
	...
	if not look_at_mode == LookAtMode.GROUP:
		if is_instance_valid(look_at_target):
			...
			if not look_at_target.tree_exiting.is_connected(_look_at_target_tree_exiting):
				look_at_target.tree_exiting.connect(
					_look_at_target_tree_exiting.bind(look_at_target)
				)
		else:
			_should_look_at = false
