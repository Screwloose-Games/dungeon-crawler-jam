...
func animate_day(start_at = 0.0, _dummy = false):
	...
	animation_tween.tween_method(
		set_day_time, start_at, 1.0, day_length_in_seconds * (1.0 - start_at)
	)
...
func animate_night(start_at = 0.0, bedtime_penalty = false):
	...
	animation_tween = create_tween()
	animation_tween.tween_method(
		set_night_time, start_at, 1.0, night_length_in_seconds * (1.0 - start_at)
	)
...
func set_day_time(time: float):
	...
	if is_raining:
		...
		$WorldEnvironment.environment.background_energy_multiplier = max(
			0.5 + $DirectionalLight3DSun.light_energy * 0.5, $DirectionalLight3DMoon.light_energy
		)
