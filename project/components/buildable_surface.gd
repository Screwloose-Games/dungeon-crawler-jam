...
func ready_structure_building(idx):
	...
	var w = int(
		StructureManager.structure_data[idx][StructureManager.StructureFields.STRUCTURE_WIDTH]
	)
	var h = int(
		StructureManager.structure_data[idx][StructureManager.StructureFields.STRUCTURE_DEPTH]
	)
...
func move_cursor_3d():
	...
	var intersect: Variant = plane.intersects_ray(
		camera.project_ray_origin(position_screen), camera.project_ray_normal(position_screen)
	)
	if intersect:
		intersect.x = round(intersect.x + 0.5)
		intersect.y = 0  #round(intersect.y)
		intersect.z = round(intersect.z + 0.5)
		selection_placeholder.position = (
			intersect + Vector3(-0.5 + position.x, 0, -0.5 + position.z)
		)
		...
		if building_rect:
			...
			can_build = surface_check(
				coords,
				building_rect.size.x,
				building_rect.size.y,
				StructureManager.VALID_TILE_TYPES[StructureManager.structure_data[building_idx][
					StructureManager.StructureFields.GROUND_BEFORE
				]],
				Vector2i.ZERO
			)
			if can_build:
				structure_placeholder.get_node("SelectionPlaceholder").mesh.material.albedo_color = (
					Color.CHARTREUSE
				)
				structure_placeholder.get_node("SelectionPlaceholder/SelectionPlaceholder").mesh.material.albedo_color = (
					Color.CHARTREUSE
				)
			...
			if is_valid:
				...
				if coords in tile_to_structure_idx:
					...
					HUDCanvasLayer.Singleton.instance.set_tool_tip(
						StructureManager.structure_data[structure.structure][
							StructureManager.StructureFields.STRUCTURE_NAME
						],
						position_screen
					)
			else:
...
func create_initial_build():
	if initial_build:
		for record in initial_build.split("\n"):
			...
			var w = int(
				StructureManager.structure_data[building_idx][
					StructureManager.StructureFields.STRUCTURE_WIDTH
				]
			)
			var h = int(
				StructureManager.structure_data[building_idx][
					StructureManager.StructureFields.STRUCTURE_DEPTH
				]
			)
			building_rect = Rect2i(0, 0, w, h)
...
func build_structure():
	var coords = Vector2i(
		int(structure_placeholder.position.x), int(structure_placeholder.position.z)
	)
...
func _build_structure(building_idx, coords, building_rect, skip_resource_consumption = false):
	...
	var file_name = StructureManager.structure_data[building_idx][
		StructureManager.StructureFields.STRUCTURE_MODEL
	]
	var directory_name = file_name.rsplit(".", true, 1)[0]
	var visual_instance = null
	var visual_instance_scene = load(
		"res://assets/3d/structures/" + directory_name + "/" + file_name
	)
	if visual_instance_scene:
		visual_instance = visual_instance_scene.instantiate()
		visual_instance.position = (
			Vector3(coords.x, 0, coords.y)
			+ Vector3(building_rect.size.x / 2.0, 0, building_rect.size.y / 2.0)
		)
	...
	surface_check(
		...
		Vector2i.ZERO,
		StructureManager.VALID_TILE_TYPES[StructureManager.structure_data[building_idx][
			StructureManager.StructureFields.GROUND_AFTER
		]]
	)

	var new_structure = StructureManager.BuiltStructure.new(
		self,
		coords,
		building_idx,
		EnvironmentManager.environment_model.day,
		StructureManager.StructureStatus.JUST_CREATED,
		visual_instance
	)
	built_structures_local.append(new_structure)
	register_tiles(
		len(built_structures_local) - 1, coords, building_rect.size.x, building_rect.size.y
	)
...
func surface_check(
	origin: Vector2i, w: int, h: int, required_kind: Vector2i, update_kind: Vector2i
) -> bool:
	for ix in w:
		for iy in h:
			var coords := origin + Vector2i(ix, iy)
