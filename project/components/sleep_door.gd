...
func _on_day_cycle_update(time: float):
	...
	if time >= time_to_sleep:
		...
		SceneTransitionManager.change_position_with_transition(
			player, wake_up_location, SceneManager.FORCE_SLEEP_TRANSITION
		)
...
func _on_interacted(player: Player):
	...
	SceneTransitionManager.change_position_with_transition(
		player, player, SceneManager.FADE_TRANSITION
	)
